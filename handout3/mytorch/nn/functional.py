...
class Pow(Function):
    ...
    def forward(ctx,x,exp):
        assert type(x) == tensor.Tensor
        assert type(exp) == int
        x_data = x.data
        y_data = np.power(x_data, exp)

        ctx.save_for_backward(x, tensor.Tensor(exp))
        
        requires_grad = x.requires_grad
        y = tensor.Tensor(y_data, requires_grad=requires_grad,
                                           is_leaf=not requires_grad)
        return y

    @staticmethod
    def backward(ctx,grad_output):
        x_t, exp_t = ctx.saved_tensors
        x, exp = x_t.data = x_t.data, exp_t.data
        #dy/dx = exponent * x^(exponent - 1)
        grad_x = exp * np.power(x, exp - 1)
        grad_x = grad_x * grad_output.data
        
        # 特殊处理：x=0 且 exponent<=0 时，梯度为inf/nan，替换为0（避免计算异常）
        grad_x = np.nan_to_num(grad_x, nan=0.0, posinf=0.0, neginf=0.0)
...
class Slice(Function):
    ...
    def forward(ctx,x,indices):
        ...
        x_data = x.data
        s = x_data[indices]
        ctx.save_for_backward(tensor.Tensor(indices), x)
        requires_grad = x.requires_grad
        return tensor.Tensor(s, requires_grad=requires_grad, is_leaf=not requires_grad)
        

    @staticmethod
    def backward(ctx,grad_output):
        indices_t, x_t = ctx.saved_tensors     
        indices, x = indices_t.data, x_t.data
        if indices.ndim != 0:
...
class Cat(Function):
    ...
    def forward(ctx,dim, *args):
        ...
        cat_arr = np.concatenate(arrs, dim)
        cat_t = tensor.Tensor(cat_arr, requires_grad=requires_grad, is_leaf=not requires_grad)
        slice_lens = np.array([s.shape[dim] for s  in arrs])
        ctx.save_for_backward(tensor.Tensor(slice_lens), tensor.Tensor(dim))
        return cat_t

    @staticmethod
    def backward(ctx,grad_output):
        slice_lens_t, dim_t = ctx.saved_tensors
        slice_lens, dim =  slice_lens_t.data, dim_t.data
        slices = []
        start = 0
        for len in slice_lens:  
          indics = tuple(range(start, start + len))
          slice = np.take(grad_output.data, indics, dim) 
          slices.append(tensor.Tensor(slice))    
          start += len
        return *slices,
