...
class RNNUnit(Module):
    ...
    def forward(self, input, hidden = None):
        ...
        t = input @ self.weight_ih.T() + self.bias_ih +  hidden @ self.weight_hh.T() + self.bias_hh
        return self.act(t)
...
class TimeIterator(Module):
    ...
    def forward(self,input,hidden = None):
        ...
        for t in range(steps):
            ...
            timestep_input = data[start: start+batch_size]
            timestep_output = self.unit(timestep_input, hidden)
            if t < steps - 1:
                next_batch_size = batch_sizes[t + 1]
                hidden = timestep_output[:next_batch_size]
                if(next_batch_size < batch_size):
                    final_hidden.append(timestep_output[next_batch_size:])
            else:
                final_hidden.append(timestep_output)
            output.append(timestep_output)
            start += batch_size
        output = tensor.cat(output)   
        final_hidden = tensor.cat(final_hidden[::-1])
        return PackedSequence(output, sorted_indices, batch_sizes), final_hidden
