...
class CNN_SimpleScanningMLP:
    ...
    def init_weights(self, weights):
        ...
        cw1 = w1.reshape(self.conv1.out_channel, self.conv1.kernel_size, self.conv1.in_channel).transpose(0, 2, 1)
        cw2 = w2.reshape(self.conv2.out_channel, self.conv2.kernel_size, self.conv2.in_channel).transpose(0, 2, 1)
        cw3 = w3.reshape(self.conv3.out_channel, self.conv3.kernel_size, self.conv3.in_channel).transpose(0, 2, 1)        
        self.conv1.weight =Tensor(cw1,requires_grad=True, is_parameter=True)
        self.conv2.weight =Tensor(cw2,requires_grad=True, is_parameter=True)
        self.conv3.weight =Tensor(cw3,requires_grad=True, is_parameter=True)
...
class CNN_DistributedScanningMLP:
    ...
    def init_weights(self, weights):
        ...
        # Make sure to not add nodes to the comp graph!
        cw1 = np.transpose(np.reshape(np.transpose(w1.T[:, :2]), (2, 8, 24))[:, :2, :], (0, 2, 1))
        cw2 = np.transpose(np.reshape(np.transpose(w2.T[:, :8]), (8, 4, 2))[:, :2, :], (0, 2, 1))
        cw3 = np.transpose(np.reshape(np.transpose(w3.T), (4, 2, 8)), (0, 2, 1))
        self.conv1.weight =Tensor(cw1, requires_grad=True, is_parameter=True)
        self.conv2.weight =Tensor(cw2, requires_grad=True, is_parameter=True)
        self.conv3.weight =Tensor(cw3, requires_grad=True, is_parameter=True)
